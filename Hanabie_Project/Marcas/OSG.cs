...
namespace Hanabie_Project.Marcas
{
    internal class OSG : Tools
    {
        ...
        public string KeiLetters { get; set; }
        public string KubushitaLetters { get; set; }
        public static List<OSG> LoadOSGXML()
        {
            string xmlFilePath = "OSG-Endmill.xml";
            ...
            public static List<OSG> LoadOSGXMLBall()
            {
                string xmlFilePath = "OSG-BallEndmill.xml";
                ...
                var products = xmlDoc.Descendants("Table")
                                     ...
                                     .Select(tr =>
                                     {
                                         var secondTH = tr.Elements("TH").ElementAtOrDefault(1)?.Value;
                                         var (kado, kubushita, kei, keiLetters, kubushitaLetters, isBallEndMill) = ExtractKadoKubushitaKei(secondTH);

                                         float.TryParse(tr.Elements("TD").ElementAtOrDefault(0)?.Value, out float hachou);
                                         int.TryParse(tr.Elements("TD").ElementAtOrDefault(1)?.Value, out int zenchou);
                                         int.TryParse(tr.Elements("TD").ElementAtOrDefault(2)?.Value, out int shanko);
                                         ...
                                         return new OSG
                                         {
                                             ...
                                             Zenchou = zenchou,
                                             Shanko = shanko,
                                             Page = tr.Elements("TD").ElementAtOrDefault(3)?.Value,
                                             Mark = Marks.OSG.ToString(),
                                             Type = isBallEndMill ? Types.ボールエンドミル.ToString() : Types.ラジアスエンドミル.ToString(),
                                             Laminas = isBallEndMill ? 2 : 0,
                                         };
                                     })
                                     .ToList();
        ...
        private static (float kado, float kubushita, float kei, string keiLetters, string kubushitaLetters, bool isBallEndMill) ExtractKadoKubushitaKei(string input)
        {
            var values = input.Split(new[] { '×' }, StringSplitOptions.RemoveEmptyEntries).Select(v => v.Trim()).ToArray();
            float.TryParse(values.ElementAtOrDefault(0)?.TrimStart('R'), out float kado);
            float.TryParse(values.ElementAtOrDefault(1), out float kubushita);

            var keiMatches = Regex.Match(values.ElementAtOrDefault(2) ?? "", @"(?<number>[-+]?\d*\.?\d+)(?<letters>[a-zA-Z]*)");
            float.TryParse(keiMatches.Groups["number"].Value, out float kei);
            string keiLetters = keiMatches.Groups["letters"].Value;

            var kubushitaMatches = Regex.Match(values.ElementAtOrDefault(1) ?? "", @"(?<number>[-+]?\d*\.?\d+)(?<letters>[a-zA-Z]*)");
            string kubushitaLetters = kubushitaMatches.Groups["letters"].Value;

            bool isBallEndMill = values.Length == 2;
            if (isBallEndMill)
            {
                kei = kado * 2;
            ...
            return (kado, kubushita, kei, keiLetters, kubushitaLetters, isBallEndMill);
        }
